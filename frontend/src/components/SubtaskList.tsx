...
import { DeleteOutlined, HolderOutlined, PlusOutlined, EditOutlined, CheckOutlined, CloseOutlined } from '@ant-design/icons';
...
import { CREATE_SUBTASK, UPDATE_SUBTASK, DELETE_SUBTASK, REORDER_SUBTASKS } from '../lib/queries';
...
function SortableSubtaskItem({
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`group flex items-center justify-between gap-2 p-2 rounded-lg border transition-colors ${
        isDragging
      ...
      cache.modify({
        ...
        fields: {
          subtasks(_, { toReference }) {
            return reordered.map((sub: Subtask) => toReference(sub));
          },
        },
      });
    },
